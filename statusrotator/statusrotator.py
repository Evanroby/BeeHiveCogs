...
class StatusRotator(commands.Cog):
    def __init__(self, bot):
        ...
        self.statuses = [
            lambda: f"{len(self.bot.guilds)} server{'s' if len(self.bot.guilds) != 1 else ''} | {len(self.bot.users):,} user{'s' if len(self.bot.users) != 1 else ''}" + (" | beehivesafety.com" if self.bot.user.id == 1152805502116429929 else ""),
